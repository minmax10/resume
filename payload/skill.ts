export default {
  disable: false,
  title: 'SKILL',
  list: [
    {
      title: 'AML/STR',
      items: [
        { title: 'STR 정책/내규 운영', level: 3 },
        { title: '룰 기반 탐지', level: 3 },
        { title: '결과 검증', level: 3 },
        { title: '보고 품질 관리', level: 3 },
        { title: '운영 프로세스 표준화', level: 3 },
        { title: 'CTR 운영', level: 2 },
      ],
    },
    {
      title: 'Data/Query',
      items: [
        { title: 'SQL(룰 추출/검증)', level: 3 },
        { title: '대용량 쿼리 튜닝', level: 3 },
        { title: '데이터 정제/추출', level: 3 },
        { title: '배치 흐름 이해', level: 2 },
        { title: '데이터 정합성 검증', level: 3 },
      ],
    },
    {
      title: 'Platform',
      items: [
        { title: 'Elasticsearch 기반 모니터링', level: 2 },
        { title: '컨테이너 환경 운영/연동', level: 2 },
        { title: 'ETL 개발', level: 2 },
      ],
    },
    {
      title: 'Delivery',
      items: [
        { title: '전산 개발 요건 정의', level: 3 },
        { title: '테스트(UAT)', level: 3 },
        { title: '이슈 트래킹', level: 2 },
        { title: '릴리즈 리스크 관리', level: 2 },
      ],
    },
    {
      title: 'Collaboration',
      items: [
        { title: '다부서 조율', level: 3 },
        { title: '해외 협력사 커뮤니케이션', level: 2 },
        { title: '프로세스 표준화', level: 3 },
      ],
    },
  ],
}
